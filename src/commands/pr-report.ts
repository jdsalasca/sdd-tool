...
export async function runPrReport(): Promise<void> {
  ...
  if (!projectName) {
    printError("SDD-1341", "Project name is required.");
    return;
  }
  ...
  try {
    ...
    printError("SDD-1342", (error as Error).message);
    return;
  }
  ...
  if (!prId) {
    printError("SDD-1343", "PR ID is required.");
    return;
  }
  let prDir: string;
  try {
    ...
    printError("SDD-1344", (error as Error).message);
    return;
  }
  if (!fs.existsSync(prDir)) {
    printError("SDD-1345", `PR review not found at ${prDir}`);
    return;
  }
